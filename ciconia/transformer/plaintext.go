...
var (
	...
	colorMarkerPattern = regexp.MustCompile("\x00color_open_(#[0-9A-Fa-f]{6})\x00([^\x00]*)\x00color_close\x00")
)
...
func (t *HtmlTransformer) Transform(elements []dialogue.DialogueElement) string {
	var buf strings.Builder
	for i := 0; i < len(elements); i++ {
		if ct, ok := elements[i].(*ast.ColoredText); ok {
			buf.WriteString("\x00color_open_")
			buf.WriteString(ct.Hex)
			buf.WriteString("\x00")
			buf.WriteString(ct.Text)
			buf.WriteString("\x00color_close\x00")
			continue
		}
		if te, ok := elements[i].(dialogue.TextElement); ok {
			buf.WriteString(te.GetText())
	...
	text = colorMarkerPattern.ReplaceAllString(text, `<span style="color:$1">$2</span>`)
	return text
}
