package decoder

import (
	"bytes"
	"compress/zlib"
	"encoding/binary"
)

var keyTable = [256]byte{
	,
}

func Encode(data []byte) ([]byte, error) {
	originalSize := len(data)

	pass1 := make([]byte, originalSize)
	for i := 0; i < originalSize; i++ {
		pass1[i] = keyTable[data[i]^pass1XorB] ^ pass1XorA
	}

	var compressed bytes.Buffer
	w := zlib.NewWriter(&compressed)
	if _, err := w.Write(pass1); err != nil {
		return nil, err
	}
	if err := w.Close(); err != nil {
		return nil, err
	}

	compressedData := compressed.Bytes()
	compressedSize := len(compressedData)

	pass2 := make([]byte, compressedSize)
	for i := 0; i < compressedSize; i++ {
		pass2[i] = keyTable[compressedData[i]^pass2XorB] ^ pass2XorA
	}

	var buf bytes.Buffer
	buf.WriteString("ONS2")
	binary.Write(&buf, binary.LittleEndian, uint32(compressedSize))
	binary.Write(&buf, binary.LittleEndian, uint32(originalSize))
	binary.Write(&buf, binary.LittleEndian, uint32(110))
	buf.Write(pass2)

	return buf.Bytes(), nil
}
