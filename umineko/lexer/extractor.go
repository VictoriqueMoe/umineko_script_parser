...
type (
	...
	ExtractedQuote struct {
		...
		AudioTextMap map[string][]dialogue.DialogueElement
		Episode      int
		ContentType  string
		Truth        TruthFlags
...
func (e *QuoteExtractor) extractFromDialogue(d *ast.DialogueLine) *ExtractedQuote {
	...
	var audioTextMap map[string][]dialogue.DialogueElement
	if len(audioIDs) > 1 {
...
func hasWordsBeforeVoice(elements []dialogue.DialogueElement) bool {
...
func buildAudioTextMap(elements []dialogue.DialogueElement) map[string][]dialogue.DialogueElement {
	result := make(map[string][]dialogue.DialogueElement)
	var currentAudioID string
	var currentFragment []dialogue.DialogueElement

	var walk func(elems []dialogue.DialogueElement)
	walk = func(elems []dialogue.DialogueElement) {
...
func containsVoiceCommand(elements []dialogue.DialogueElement) bool {
	for _, elem := range elements {
		switch el := elem.(type) {
		case *ast.VoiceCommand:
			return true
